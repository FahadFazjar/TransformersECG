...
class trainer(object):
    ...
    def train(self):
        ...
        self.cross_entropy = torch.nn.CrossEntropyLoss(weight=torch.tensor(np.array(list(self.cw_dict.values()))).float().to(self.device))
        ...
        for epoch in range(1, self.hparams["num_epochs"] + 1):
            ...
            if ts_f1 > best_f1:  # save best model based on best f1.
                best_f1 = ts_f1
                best_acc = ts_acc
                save_checkpoint(self.exp_log_dir, model, self.dataset, self.dataset_configs, self.hparams)
                _save_metrics(self.pred_labels, self.true_labels, self.exp_log_dir, self.dataset_configs.class_names)
